...
namespace LuxWater {
	...
	// We have to manually grab the depth texture
		...
		//	Setup UnderWaterMask and SetRenderTarget upfront tp prevent spikes.
			if (!UnderWaterMask) {
		...
		//	Render UnderWaterMask
			...
			//	Draw UnderWaterMask
				...
				//Shader.SetGlobalTexture(UnderWaterMaskPID, Texture2D.whiteTexture);
		...
		void OnRenderImage(RenderTexture src, RenderTexture dest) {
			...
			if(DoUnderWaterRendering && activeWaterVolume > -1) {
				...
				if (WaterMaterials[activeWaterVolume].GetFloat("_CausticsEnabled") == 1) {
					blitMaterial.EnableKeyword("GEOM_TYPE_FROND");
					if (WaterMaterials[activeWaterVolume].GetFloat("_CausticMode") == 1) {
                ...
                if (islinear)
				    Shader.SetGlobalColor("_Lux_UnderWaterFogColor", WaterMaterials[activeWaterVolume].GetColor("_Color").linear );
                else
                    Shader.SetGlobalColor("_Lux_UnderWaterFogColor", WaterMaterials[activeWaterVolume].GetColor("_Color"));

                Shader.SetGlobalFloat("_Lux_UnderWaterFogDensity", WaterMaterials[activeWaterVolume].GetFloat("_Density") );
				Shader.SetGlobalFloat("_Lux_UnderWaterFogAbsorptionCancellation", WaterMaterials[activeWaterVolume].GetFloat("_FogAbsorptionCancellation") );

				Shader.SetGlobalFloat("_Lux_UnderWaterAbsorptionHeight", WaterMaterials[activeWaterVolume].GetFloat("_AbsorptionHeight") );
				Shader.SetGlobalFloat("_Lux_UnderWaterAbsorptionMaxHeight", WaterMaterials[activeWaterVolume].GetFloat("_AbsorptionMaxHeight") );
